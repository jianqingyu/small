...
- (void)viewDidLoad {
    ...
    for (int i=0; i<YQNewfeatureCount; i++) {
        ...
        imageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"home0%d.jpg",i+1]];
...
- (void)setLastImageView:(UIImageView *)imageView{
    UIButton *startBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [startBtn setImage:[UIImage imageNamed:@"icon_start"] forState:UIControlStateNormal];
    [startBtn addTarget:self action:@selector(startClick) forControlEvents:UIControlEventTouchUpInside];
    [imageView addSubview:startBtn];
    [startBtn mas_makeConstraints:^(MASConstraintMaker *make) {
